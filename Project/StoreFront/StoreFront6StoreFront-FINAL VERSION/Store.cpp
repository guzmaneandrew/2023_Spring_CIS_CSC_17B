...
int Store::getNumItms() const {
    return numItems;
}

vector<Item *> Store::getItems() const {
    return items;
}

Item* Store::getItem(int i) const {
    return items[i];
}

void Store::clearItems() {
    for (int i = 0; i < items.size(); i++) {
        delete items[i];
    }
    items.clear();
...
void Store::setItem(string n, float p, int s) {
    Item *newItem = new Item(n, p, s);
    items.push_back(newItem);
    numItems++;
}

void Store::setItems(int num) {
    numItems = num;
    for (int i = 0; i < num; i++) {
        newItem();
    }
...
void Store::setItems(vector<Item *> items) {
    this->items = items;
}

void Store::setNumItems(int n) {
    numItems = n;
}

void Store::setNumItems() {
    numItems = items.size();
}
...
void Store::saveBin(string file) {
...
void Store::readBin(string file) {
