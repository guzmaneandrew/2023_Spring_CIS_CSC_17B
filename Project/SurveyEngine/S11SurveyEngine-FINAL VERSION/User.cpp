...
void User::setUsrInfo(string name, string pass) {
...
void User::answerQ(int i, Question *q) {
...
void User::singleResp(int i, Question *q) {
    string response;
...
void User::multResp(int i, Question *q) {
    string usrResps;
...
void User::wrtInResp(int i, Question *q) {
...
vector<string> User::strSplit(string s) {
    vector<string> list;
    stringstream ss(s);
    string substr;
...
void User::listSrvs(string srvDBfile) {
    int num = 1;
    readFromBin(usrSrvFile, userSrvs);
...
bool User::chkComplete(Survey * srv) {
    bool complete;
...
void User::listCompSrvs(string file) {
    int num = 1;
    bool none = true;
...
void User::prntSrvs(string srvDBfile) {
    int survNum = 0, indx = 0, validNum = 0;
...
void User::prntResp(Survey *srv, int srvIndx) {
...
int User::findRespIndx(Survey *srv, int srvIndx) {
    int numQs = 0;
...
void User::readFromBin(string file, vector<Survey *>& srvs) {
...
void User::reloadSrvs(string file, vector<Survey *> srvs) {
    //Clear binary file, output vector surveys to binary, then write to bin
    clearBin(file);
...
void User::clearBin(string file) {
