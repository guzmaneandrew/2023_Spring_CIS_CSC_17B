...
int main(int argc, char** argv) {
    ...
    //Allocate the array of structures DYNAMICALLY
    ...
    for(int mvees=0;mvees<nMovies;mvees++){//Gotta frickin indent!!!!!
        ...
        mveInfo[mvees].dirName=new char[size];  //DYNAMIC char array for director name
    ...
    for(int mvees=0;mvees<nMovies;mvees++){
        delete [] mveInfo[mvees].dirName;//de-allocate the char arrays in the Structure
    }
    delete [] mveInfo;//Now de-allocate the array of Structures
